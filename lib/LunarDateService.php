...
class LunarDateService
{
    ...
    public function date($format, string|int $timestamp = '', string $time_zone = 'Asia/Baghdad', string $tr_num = 'fa'): array|string
    {
        $T_sec = 0;
        if ($time_zone != 'local') {
            date_default_timezone_set(($time_zone === '') ? 'Asia/Baghdad' : $time_zone);
        }
        ...
        foreach ($format_extract as $sub) {
            ...
            switch ($sub) {
                ...
                case 'F':
                case 'M':
                    $hjMonth = ['', 'محرم', 'صفر', 'ربيع أول', 'ربيع ثاني',
     ...
     * @return array|string
     */
    public function tr_num($str, string $mod = 'en', string $mf = '٫'): array|string
    {
    ...
    {
        $convertToGre = SolarDate::solar_to_gregorian($s_y, $s_m, $s_d);
     ...
     * @return bool
     */
    public function isLeapYearG($year): bool
    {
    ...
    public function lunar_to_gregorian(string|int $l_y, string|int $l_m, string|int $l_d, string $mod = ''): array|string
    {
        ...
        if ($MonthDaysLunar[$l_m] < $l_d || $l_m > 12) {
            return false;
        }
    ...
    public function lunar_to_solar(string|int $l_y, string|int $l_m, string|int $l_d, string $mod = ''): array|string
    {
        ...
        return SolarDate::gregorian_to_solar($to_gr[0], $to_gr[1], $to_gr[2], $mod);
    }
